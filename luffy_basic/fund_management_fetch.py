            ...
            if cell.value:  # 如果单元格有值
                try:
                    translated = translator.translate(
                        cell.value, src="en", dest="ja")  # 翻译为日语
                    cell.value = translated.text  # 替换为翻译后的内容
...
if __name__ == "__main__":
    ...
    # for fioriId, urls in urls.items():
    #     print("{} is done".format(fioriId))
    #     asyncio.run(async_fetchs(urls))
    # end_time = time.time()
    # 204.5s

    # with ThreadPoolExecutor(max_workers=10) as executor:  # 最大线程数
    #     for fioriId, urls in urls.items():
    #         print("{} is done",fioriId)
    #         executor.map(asyncio.run(async_fetchs(urls)))
    # end_time = time.time()
    # 235.6s
